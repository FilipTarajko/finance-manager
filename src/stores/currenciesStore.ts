...
export const useCurrenciesStore = defineStore('currencies', () => {
  ...
  function getCurrencyNameByTransaction(
    transaction: TransactionWithoutCategoryData | TransactionWithCategoryData
  ) {
  ...
  function getCurrencyByTransaction(
    transaction: TransactionWithoutCategoryData | TransactionWithCategoryData
  ) {
    const account = accountsStore.accounts.find((acc) => acc.id == transaction.account_id)
  ...
  function updateValueOfDependentCurrencies(currency: Currency) {
    for (let i = 0; i < currencies.value.length; i++) {
      ...
      if (currencyToUpdate.base_currency_id == currency.id) {
        currencyToUpdate.value_relative_to_default =
          currency.value_relative_to_default * currencyToUpdate.value_relative_to_base
        updateValueOfDependentCurrencies(currencyToUpdate)
      }
    }
  }

  function editExistingCurrency(
    currency: Currency,
    newState: {
      name: string
    ...
    editedCurrency.value_relative_to_default =
      editedCurrency.value_relative_to_base *
      getCurrencyById(editedCurrency.base_currency_id)!.value_relative_to_default
  ...
  function createAndAddCurrency(
    ...
    currencies.value.push({
      ...
      value_relative_to_default:
        value_relative_to_base * getCurrencyById(base_currency_id)!.value_relative_to_default,
      api_name
    })
    if (create_account) {
  ...
  async function updateValuesWithApi() {
    ...
    try {
      for (let i = 0; i < currencies.value.length; i++) {
        ...
        if (
          currency.api_name &&
          base_currency?.api_name &&
          currency.id != default_currency_id.value
        ) {
          const response = await fetch(
            `https://api.vatcomply.com/rates?base=${base_currency.api_name.toUpperCase()}`
          )
          const new_value_relative_to_base =
            1 / (await response.json()).rates[currency.api_name.toUpperCase()]
          ...
          currency.value_relative_to_default =
            currency.value_relative_to_base * base_currency.value_relative_to_default
      ...
      if (changed_values_of_currencies.length) {
        snackbarStore.showSnackbarMessage(
          'green',
          `Updated rates of currencies: ${changed_values_of_currencies.join(', ')}`
        )
      } else if (updated_currencies_array.length) {
        snackbarStore.showSnackbarMessage(
          'green',
          `Rates were checked, but all were already up to date`
        )
      } else {
